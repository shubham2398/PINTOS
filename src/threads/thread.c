...
thread_unblock (struct thread *t) 
{
  ...
  list_insert_priority (&ready_list, &t->elem, "priority");
...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread) 
    list_insert_priority (&ready_list, &cur->elem, "priority");
...
/* Sets the current thread's priority to NEW_PRIORITY and yields the current
   thread if the priority of the first element of the ready_list is greater
   than the NEW_PRIORITY. */
void
...
thread_donate_priority (struct thread *t1, struct thread *t2, struct lock *lock)
{
  ...
  list_remove(&t2->elem);
  list_insert_priority(&ready_list,&t2->elem, "priority");
...
/* Sets current thread's wake up tick value to TICK i.e. thread will wake up after timer ticks equals TICK*/
void
...
bool
thread_try_wakeup(struct thread * t, int64_t tick){
